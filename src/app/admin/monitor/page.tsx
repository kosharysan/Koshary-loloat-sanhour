    ...
    <div className={`min-h-screen selection:bg-amber-500 selection:text-slate-950 pb-24 transition-colors duration-200 ${
      ...
      <main className="max-w-7xl mx-auto px-3 sm:px-6 pt-4 sm:pt-6 space-y-4 sm:space-y-6">
        ...
        <section className={`relative rounded-3xl p-3 sm:p-5 lg:p-6 space-y-3.5 sm:space-y-4 border-2 transition-all overflow-hidden ${
          isLight
            ? 'bg-gradient-to-br from-[#0c1322] via-[#111c30] to-[#0a0f1d] border-amber-400/50 shadow-2xl shadow-slate-900/15 ring-1 ring-amber-400/30 text-white'
          ...
          {/* خلفيات ضوئية جمالية للبلوك الموحد */}
          <div className="absolute top-0 left-1/4 w-96 h-48 bg-amber-500/10 rounded-full blur-3xl pointer-events-none" />
          ...
          <div className="relative z-10 pt-3 sm:pt-3.5 border-t border-slate-750/80 space-y-3 sm:space-y-3.5">
            ...
            <div className="flex items-center justify-between gap-2 pb-0.5">
              ...
              <span className="text-[11px] font-black px-2.5 py-0.5 rounded-full border bg-slate-950/80 text-amber-300 border-amber-500/30 shadow-2xs">
                النتائج: <strong className="font-mono font-black text-white">{filteredOrders.length}</strong> من {orders.length}
              </span>
            </div>
            ...
            <div className="relative">
              <input
                ...
                className="w-full py-3 px-4 pr-11 pl-10 rounded-xl sm:rounded-2xl border-2 text-xs sm:text-sm font-bold transition focus:outline-none focus:ring-2 bg-slate-950/90 border-slate-700/90 text-white placeholder-slate-400 focus:border-amber-400 focus:ring-amber-400/20 hover:border-slate-600 shadow-inner"
              />
              <Search className="absolute top-3.5 right-3.5 w-4 h-4 text-amber-400" />
              {searchQuery && (
                <button
                  onClick={() => setSearchQuery('')}
                  className="absolute top-3 left-3 p-1 rounded-lg transition hover:bg-slate-800 text-slate-400 hover:text-white"
                >
                  <X className="w-4 h-4" />
                </button>
              )}
