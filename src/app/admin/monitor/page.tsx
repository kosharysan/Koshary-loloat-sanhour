    ...
    <div className={`min-h-screen selection:bg-amber-500 selection:text-slate-950 pb-24 transition-colors duration-200 ${
      ...
      <header className={`sticky top-0 z-40 backdrop-blur-xl border-b px-2.5 sm:px-6 py-2.5 sm:py-3 transition-colors duration-200 ${
        ...
        <div className="max-w-7xl mx-auto flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2.5 sm:gap-3">
          ...
          {/* Action Buttons & Desktop Clock: 100% visible on all screens including mobile */}
          <div className="flex flex-wrap items-center justify-end sm:justify-end gap-1.5 sm:gap-2.5 w-full sm:w-auto">
            ...
            <button
              ...
              className={`py-1.5 sm:py-2 px-2 sm:px-3 rounded-xl border text-xs font-bold transition flex items-center justify-center gap-1 cursor-pointer disabled:opacity-50 active:scale-95 shadow-xs shrink-0 ${
                isLight
            ...
            >
              ...
              <span className="hidden xs:inline sm:inline whitespace-nowrap">تحديث</span>
            </button>

            {/* Sound Alerts Toggle */}
            <button
              onClick={() => setSoundEnabled(!soundEnabled)}
              className={`py-1.5 sm:py-2 px-2 sm:px-3 rounded-xl border text-xs font-bold transition flex items-center justify-center gap-1 cursor-pointer active:scale-95 shadow-xs shrink-0 ${
                soundEnabled
                  ? isLight
            ...
            >
              ...
              <span className="hidden sm:inline whitespace-nowrap">{soundEnabled ? 'صوت' : 'صامت'}</span>
            </button>
            ...
            >
              {isLight ? (
                <>
                  ...
                  <span className="hidden sm:inline whitespace-nowrap">نهاري</span>
                </>
              ) : (
                <>
                  <Sun className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-amber-400 shrink-0" />
                  <span className="hidden sm:inline whitespace-nowrap">ليلي</span>
                </>
              )}
            </button>

            {/* Logout Button (خروج) - Guaranteed visible on all screens */}
            <button
              onClick={handleLogout}
              className={`py-1.5 sm:py-2 px-2.5 sm:px-3.5 rounded-xl border text-xs font-bold transition flex items-center justify-center gap-1 cursor-pointer active:scale-95 shadow-xs shrink-0 ${
                isLight
