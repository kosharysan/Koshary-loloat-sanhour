...
export default function OrderMonitorPage() {
  ...
  return (
    <div className={`min-h-screen selection:bg-amber-500 selection:text-slate-950 pb-24 transition-colors duration-200 ${
      ...
      {/* نافذة تأكيد تقفيل الوردية وتصفير الفواتير - تصميم نظيف وعصري */}
      {isShiftModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-4 bg-black/80 backdrop-blur-sm animate-in fade-in duration-150 font-sans">
          <div className={`relative w-full max-w-lg max-h-[90vh] overflow-y-auto rounded-2xl p-5 sm:p-6 border shadow-2xl space-y-4 font-sans ${
            isLight
            ...
            {/* توقيت الوردية */}
            <div className={`px-3.5 py-2.5 rounded-xl border flex items-center justify-between text-xs font-semibold ${
              isLight
            ...
            <div className="grid grid-cols-3 gap-2">
              ...
              <div className={`p-3 rounded-xl border text-center ${
                ...
                <span className="text-xl font-bold text-amber-500 block">{shiftSummary.totalOrders}</span>
              </div>

              {/* المؤكد */}
              <div className={`p-3 rounded-xl border text-center ${
                isLight ? 'bg-emerald-50/60 border-emerald-200 text-emerald-950' : 'bg-emerald-500/10 border-emerald-500/25'
              }`}>
                <span className="text-[11px] font-bold text-emerald-600 dark:text-emerald-400 block mb-1">✅ المؤكدة</span>
                <span className="text-xl font-bold text-emerald-500 block">{shiftSummary.confirmedOrders}</span>
              </div>

              {/* الملغي */}
              <div className={`p-3 rounded-xl border text-center ${
                isLight ? 'bg-rose-50/60 border-rose-200 text-rose-950' : 'bg-rose-500/10 border-rose-500/25'
              }`}>
                <span className="text-[11px] font-bold text-rose-600 dark:text-rose-400 block mb-1">🚫 الملغية</span>
                <span className="text-xl font-bold text-rose-500 block">{shiftSummary.cancelledOrders}</span>
              </div>
            </div>
            ...
            <div className={`p-4 rounded-xl border flex items-center justify-between ${
              isLight 
                ...
                : 'bg-gradient-to-r from-emerald-950/50 to-slate-900 border-emerald-500/30'
            }`}>
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  <span className="text-xs font-bold text-emerald-600 dark:text-emerald-400 block">💰 صافي مبيعات الوردية</span>
                  <span className="text-[11px] text-slate-400 block">إجمالي مبيعات الفواتير المؤكدة</span>
                </div>
              </div>
              <div className="text-left">
                <span className="text-2xl font-black text-emerald-500">
                  {shiftSummary.totalRevenue.toLocaleString()}
                </span>
            ...
            <div className="grid grid-cols-2 gap-2">
              ...
              <div className={`p-3 rounded-xl border flex items-center justify-between ${
                isLight ? 'bg-sky-50/60 border-sky-200' : 'bg-sky-500/10 border-sky-500/20'
              }`}>
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    <span className="text-xs font-bold text-sky-600 dark:text-sky-400 block">طلبات الدليفري</span>
                    <span className="text-[10px] text-slate-400 block">توصيل منزلي</span>
                  </div>
                </div>
                <div className="text-lg font-bold text-sky-500">
                  {shiftSummary.deliveryCount} <span className="text-[11px] font-normal text-slate-400">طلب</span>
                </div>
              </div>

              {/* الصالة وتيك أواي (بجانب الدليفري) */}
              <div className={`p-3 rounded-xl border flex items-center justify-between ${
                isLight ? 'bg-purple-50/60 border-purple-200' : 'bg-purple-500/10 border-purple-500/20'
              }`}>
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    <span className="text-xs font-bold text-purple-600 dark:text-purple-400 block">صالة وتيك أواي</span>
                    <span className="text-[10px] text-slate-400 block">استلام مباشر</span>
                  </div>
                </div>
                <div className="text-lg font-bold text-purple-500">
                  {shiftSummary.pickupCount} <span className="text-[11px] font-normal text-slate-400">طلب</span>
                </div>
              </div>
            </div>
            ...
            <div className={`p-3 rounded-xl border space-y-2 ${
              ...
              <div className="grid grid-cols-3 gap-2 text-center text-xs">
                <div className={`p-2 rounded-lg border ${
                  ...
                  <span className="font-bold text-emerald-500 block mt-0.5">{shiftSummary.cashAmount.toLocaleString()} ج.م</span>
                </div>
                ...
                <div className={`p-2 rounded-lg border ${
                  ...
                  <span className="font-bold text-amber-500 block mt-0.5">{shiftSummary.walletAmount.toLocaleString()} ج.م</span>
                </div>
                ...
                <div className={`p-2 rounded-lg border ${
                  ...
                  <span className="font-bold text-cyan-500 block mt-0.5">{shiftSummary.instapayAmount.toLocaleString()} ج.م</span>
                </div>
              </div>
            </div>
            ...
            {shiftSummary.pendingOrders > 0 && (
              <div className={`p-3 rounded-xl border flex items-center gap-2.5 text-xs font-bold ${
                isLight ? 'bg-amber-50 border-amber-300 text-amber-950' : 'bg-amber-500/10 border-amber-500/25 text-amber-300'
              }`}>
                <AlertTriangle className="w-4 h-4 text-amber-500 shrink-0" />
                <span>تنبيه: يوجد {shiftSummary.pendingOrders} طلبات قيد الانتظار سيتم ترحيلها مع هذه الوردية.</span>
              </div>
            )}

            {/* تنبيه تصفير الشاشة */}
            <div className={`p-3 rounded-xl border flex items-start gap-2 text-xs leading-relaxed ${
              isLight ? 'bg-blue-50/70 border-blue-200 text-blue-900' : 'bg-blue-950/30 border-blue-500/20 text-blue-200'
            }`}>
              <ShieldCheck className="w-4 h-4 text-blue-400 shrink-0 mt-0.5" />
              <span>
                عند التأكيد سيتم تصفير شاشة المتابعة فوراً للوردية الجديدة رقم #{currentShiftNumber + 1} مع نقل كافة الفواتير لأرشيف <strong>فواتير الورديات</strong>.
              </span>
            </div>
            ...
            <div className="flex gap-2.5 pt-1">
              ...
              <button
                ...
                onClick={() => setIsShiftModalOpen(false)}
                disabled={isClosingShift}
                className={`py-3 px-4 rounded-xl text-xs font-bold transition cursor-pointer active:scale-[0.98] border ${
                  isLight
                    ? 'bg-slate-100 hover:bg-slate-200 text-slate-700 border-slate-200'
                    : 'bg-slate-800 hover:bg-slate-700 text-slate-300 border-slate-700'
                }`}
